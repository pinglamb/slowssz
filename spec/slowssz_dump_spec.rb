...
RSpec.describe Slowssz do
  describe '.dump' do
    ...
    it 'bitvector FTF' do
      expect(Slowssz::Marshal.dump(Slowssz::BitVectorType.of(size: 3).new([false, true, false]))).to eq(
        ['02'].pack('H*')
      )
    end

    it 'bitlist FTF' do
      expect(Slowssz::Marshal.dump(Slowssz::BitListType.of(limit: 4).new([false, true, false]))).to eq(
        ['0a'].pack('H*')
      )
    end
    ...
    it 'odd bitlist filled' do
      expect(Slowssz::Marshal.dump(new_bit_list_from_bytes(513, 'ff' * 64, [true]))).to eq(
        ['ff' * 64 + '03'].pack('H*')
      )
    end
    ...
    it 'empty var element list' do
      expect(Slowssz::Marshal.dump(Slowssz::ListType.of(type: VarTestContainer, limit: 8).new([]))).to eq(
        [''].pack('H*')
      )
    end
